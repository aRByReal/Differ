...
def fm(vertices, L):
    ...
    rand_string, cut = m2.fm2(vertices,L)
    return rand_string, cut

def fm2(vertices, L, passes):
    
    rand_string, cut , passes= m2.fm3(vertices,L,passes)
...
def GLS(N,T):
    ...
    # If T=0 we run the algorithm for #Pa passes      
    else:
        passes = 0
        while passes < Pa:
            A = random.sample(range(0,50), 2)
...
def plotpert():
    pert = find_pertubation_size(0,5,3)
...
def main():
    ...
    #print(random.sample(range(0,50), 2))
    print(mls)
    #IdealSize = plotpert()
    #IdealSize = 17
    #print(IdealSize)
